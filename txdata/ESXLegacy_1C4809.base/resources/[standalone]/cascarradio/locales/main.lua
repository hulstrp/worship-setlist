Language = { 
    ['EN'] = { --english
        ['name'] = 'Radio Options',
        ['volume_label'] = 'Volume',
        ['volume_help'] = 'Change the volume of the radio',
        ['volume_icon'] = 'volume-high',
        ['custom_url_label'] = 'Custom URL',
        ['custom_url_help'] = 'Enter a custom youtube URL to play in the radio',
        ['ustom_url_icon'] = 'youtube'
    },

    ['PT'] = { --portuguese
        ['name'] = 'Opções da Rádio',
        ['volume_label'] = 'Volume',
        ['volume_help'] = 'Mudar Volume da rádio',
        ['volume_icon'] = 'volume-high',
        ['custom_url_label'] = 'URL Personalizado',
        ['custom_url_help'] = 'Introduz um URL de Youtube personalizado',
        ['ustom_url_icon'] = 'youtube'
    },

    ['SR'] = { --serbian
        ['name'] = 'Radio Podesavanja',
        ['volume_label'] = "Zvuk",
        ['volume_help'] = "Promeni zvuk radija",
        ['volume_icon'] = "volume-high",
        ['custom_url_label'] = "Youtube URL",
        ['custom_url_help'] = "Unesi youtube URL da pustis na radiju",
        ['custom_url_icon'] = "youtube"
    },

    ['PL'] = { --polish
        ['name'] = 'Radio samochodowe',
        ['volume_label'] = 'Głośność',
        ['volume_help'] = 'Zmiana głośności radia',
        ['volume_icon'] = 'volume-high',
        ['custom_url_label'] = 'Link z YouTube',
        ['custom_url_help'] = 'Wklej tutaj link do utworu, który chcesz odtworzyć w radiu.',
        ['custom_url_icon'] = 'youtube'
    },

    ['NL'] = { --Nederlands
	['name'] = 'Radio Opties',
	['volume_label'] = 'Volume',
	['volume_help'] = 'Verander het volume van de radio',
	['volume_icon'] = 'volume-high',
	['custom_url_label'] = 'Aangepaste URL',
	['custom_url_help'] = 'Voer een aangepaste YouTube-URL in om af te spelen op de radio',
	['custom_url_icon'] = 'youtube'
    },
}